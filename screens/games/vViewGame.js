...
export default function ViewGame({ navigation }) {
    ...
    const handleInitiation = async () => {
        try {
            ...
            /* await axios.post('http://192.168.113.241:5000/getById', { _id: gameId, TABLE_ID: '2' })
                .then(response => {
                    setGame(response.data);
                }).catch(error => console.log("ERRO 42 " + error.response.data)) */
        } catch (error) {
            ...
            <View style={styles.backgroundColorMain}>
                ...
                <View>
                    ...
                    {reviews?.map(i => {
                        ...
                        return (
                            <Pressable onPress={() => {
                                data = { gameId: i.value['_id'], userId: userId }
                                console.log(data)
                                navigation.navigate('ViewGame', )
                            }}>
                                <View style={gameCard.card}>
                                    <Image source={require('../../assets/user_placeholder.jpg')} style={gameCard.image} />
                                    <View><Text style={gameCard.userTitle}>{i.value['reviewTitle']}</Text></View>
                                    <View><Text style={gameCard.description}>{i.value['review']}</Text></View>
                                </View>
                            </Pressable>
                        )
                    })}
